...
const updateLS = (name,place,birth,role,battingstyle,bowlingstyle) => {
    ...
    var playerList = localStorage.getItem('p-list') === null ? [] : JSON.parse(localStorage.getItem('p-list'));
    playerList.push(obj);
    localStorage.setItem("p-list", JSON.stringify(playerList));
    loadDataLS();
...
function loadDataLS(){
    const playerDataGet = JSON.parse(localStorage.getItem("p-list"));
